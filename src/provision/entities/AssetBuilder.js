...
const ID = 'provision.asset.identifier',
    PUT_METHOD = 'PUT',
    POST_METHOD = 'POST';
...
class BoxBuilder {
    constructor(ogapi, obj, url, key) {
        ...
        Object.keys(subscriptions).forEach((commsId) => {
            ...
            _this._wrappers.push(new WrapperBuilder(_this._ogapi, obj, _this._url.replace('entities', 'subscriptions').replace('/' + _this._key._value._current.value, ''), obj[SubscriptionID]._value._current.value));
        });
        Object.keys(subscribers).forEach((commsId) => {
            ...
            _this._wrappers.push(new WrapperBuilder(_this._ogapi, obj, _this._url.replace('entities', 'subscribers').replace('/' + _this._key._value._current.value, ''), obj[SubscriberID]._value._current.value));
        });
...
*/
        q.allSettled(
            ...
            defer.notify({ message: 'Creating new asset:' + _this._key._value._current.value, type: 'success', percentage: 25 });
            return _this._ogapi.Napi.post(_this._url, postObj)
                .then((res) => {
                    defer.notify({ message: 'Created asset:' + _this._key._value._current.value, type: 'success', percentage: 50 });
                        ...
                        return _this._ogapi.Napi.put(_this._urlWithKey(), putObj)
                            .then((res) => {
                                if (res.statusCode === HttpStatus.CREATED) {
                                    ...
                                    defer.notify({ message: 'asset created successfully: ' + _this._key._value._current.value, type: 'success', percentage: 75 });
    ...
    update() {
        ...
        q.allSettled(
            ...
            defer.notify({ message: 'Adding related entities to Asset:' + _this._key._value._current.value, type: 'success', percentage: 45 });
            return _this._ogapi.Napi.put(_this._url, putObj)
                .then((res) => {
                    if (res.statusCode === HttpStatus.OK) {
                        ...
                        defer.notify({ message: 'Asset updated successfully: ' + _this._key._value._current.value, type: 'success', percentage: 90 });
                        defer.resolve({ location: res.header['location'], statusCode: res.statusCode });
                    } else {
                        defer.reject({ errors: res.errors, statusCode: res.statusCode });
                    }
 ...
 */
export default class AssetBuilder extends ComplexBuilder {
