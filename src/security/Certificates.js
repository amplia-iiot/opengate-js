...
import q from 'q';
import { ADMINISTRATIVE_STATE_ENUM } from './ADMINISTRATIVE_STATE_ENUM';
import { USAGES_ENUM } from './USAGES_ENUM';
...
export default class Certificates extends Security {
    ...
    withDescription(description) {
        if (typeof description !== 'string' || description.length === 0 || description.length > max_length_description)
            throw new Error(
                'Parameter description must be a string, cannot be empty and has a maximum length of ' + max_length_description
            );
    ...
    withAdministrativeState(administrativeState) {
        ...
        if (not_found !== '') {
            throw new Error(
                "Parameter in administrativeState is not allowed. Parameter value '" +
                    JSON.stringify(not_found) +
                    "', parameters allowed in administrativeState are: '" +
                    JSON.stringify(ADMINISTRATIVE_STATE_ENUM) +
                    "'"
            );
        }
    ...
    withUsages(usages) {
        ...
        if (not_found.length !== 0) {
            throw new Error(
                "There are not any parameters allowed. Parameters value: '" +
                    JSON.stringify(not_found) +
                    "'. Usages parameters allowed are: '" +
                    JSON.stringify(USAGES_ENUM) +
                    "'"
            );
        }
        this._usages = usages;
     ...
     */
    _composeElement() {
        if (this._name === undefined || this._administrativeState === undefined || this._usages === undefined)
    ...
    update() {
        ...
        this._ogapi.Napi.put(this._buildURL(), form, undefined, this._getExtraHeaders(), this._getUrlParameters())
            .then(response => {
                ...
                if (typeof data === 'undefined') defered.reject('Cannot update the certificate');
                else defered.resolve(data, response.statusCode);
            })
            .catch(error => {
                defered.reject(error);
            });
