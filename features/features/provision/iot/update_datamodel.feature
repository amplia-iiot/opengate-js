...
|     id     |   profile_test  |
   Then response code should be: 200
    ...
    #And I can see into the result an "category[0] name" as "profile_changeExampleFlavour" NO APLICA DESDE V8
...
|     id     |   profile_test  |
   Then response code should be: 200
    And I can see into the result an "category[0] name" as "1category"
    And I can see into the result an "category[1] name" as "2category"
    And an ogapi "datamodels helper" util with... 
|                                               param                                              |
|                                         base_organization                                        |
|{"identifier":"profile_test","name":"profile_change","version":"2.0","description":"CHANGE","categories":[]}|
...
|     id     |   profile_test  |
   Then response code should be: 200
    And I can see into the result an "category[0] name" as "2category"

################################################################################################################

Scenario: Add and remove datastreams of Datamodel that exists     
    And an ogapi "datamodels helper" util with... 
|                                               param                                              |
|                                         base_organization                                        |
|{"identifier":"profile_test","name":"profile_change","version":"2.0","description":"CHANGE","categories":[]}|
...
|                                               param                                              |
|1category                                                                                 |
|{"id": "1category.example.test","name": "1category.example.test","description": "for tests","period": "PULSE","unit": {"type": "SI","label": "beats/second","symbol": "bpm"},"tags": ["test","category_1"], "schema" : { "type":"string"} }|
    And the "add datastream" with...
|                                               param                                              |
|2category                                                                                 |
|{"id": "2category.example.test","name": "2category.example.test","description": "for tests","period": "PULSE","unit": {"type": "SI","label": "beats/second","symbol": "bpm"},"tags": ["test","category_2"], "schema" : { "type":"string"} }|
    And I update it
   Then response code should be: 200
    And an ogapi "datamodels finder" util
    And I want to read a "datamodel"
...
|     id     |   profile_test  |
   Then response code should be: 200
    And I can see into the result an "category[0] name" as "1category"
    And I can see into the result an "category[0] datastream[0] id" as "1category.example.test"
    And I can see into the result an "category[1] name" as "2category"
    And I can see into the result an "category[1] datastream[0] id" as "2category.example.test"
    And an ogapi "datamodels helper" util with... 
|                                               param                                              |
|                                         base_organization                                        |
|{"identifier":"profile_test","name":"profile_change","version":"2.0","description":"CHANGE","categories":[]}|
...
|                                               param                                              |
|2category                                                                                 |
|{"id": "2category.example.test.not.remove","name": "2category.example.test.not.remove","description": "for tests","period": "PULSE","unit": {"type": "SI","label": "beats/second","symbol": "bpm"},"tags": ["test","category_2"], "schema" : { "type":"string"}}|
    And I update it
   Then response code should be: 200
    And the "remove category" "1category"
...
|     id     |   profile_test  |
   Then response code should be: 200
    And I can see into the result an "category[0] name" as "2category"
    And I can see into the result an "category[0] datastream[0] id" as "2category.example.test.not.remove"
...
| 2category |
|2category.example.test|
|{"id": "category2.example.test","name": "category2.example.test","description": "for tests and update","period": "PULSE","unit": {"type": "SI","label": "beats/second","symbol": "bpm"},"tags": ["test","category2"],"schema":{"type":"string"}}|
    And I update it
    And an ogapi "datamodels finder" util
    And I want to read a "datamodel"
   When I try to find by... 
...
|     id     |   profile_test  |
   Then response code should be: 200
    And I can see into the result an "category[0] name" as "category1"
    And I can see into the result an "category[0] datastream[0] id" as "1category.example.test"
    And I can see into the result an "category[1] name" as "2category"
    And I can see into the result an "category[1] datastream[0] id" as "category2.example.test"
################################################################################################################
...
|     id     |   profile_test  |
   Then response code should be: 200
    And I can see into the result an "category[0] name" as "1category"
    And I can see into the result an "category[0] datastream[0] id" as "test.build.datastream"
