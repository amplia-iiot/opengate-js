...
Feature: Delete and Create a device
  ...
  Scenario: I want to create a device from json file
    Given an ogapi "json flattened bulk builder" util with "organization_bulk" and "entities"
  ...
  Scenario: I want to create a device from json file - duplicated error
    ...
    Then the content of file "result.csv" must match:
      """
      statusCode;location;errors\n400;https:\/\/([\d\d\d]{1,3}\.){3}[\d\d\d]{1,3}:[\d\d\d]{1,4}\/v80\/provision\/organizations\/organization_bulk\/devices\/device_json_flattened;\[\{"code":"0x010114","message":"Entity duplicated\.","context":\[\]}]\n400;https:\/\/([\d\d\d]{1,3}\.){3}[\d\d\d]{1,3}:[\d\d\d]{1,4}\/v80\/provision\/organizations\/organization_bulk\/devices\/device_json_flattened_withCM;\[\{"code":"0x010000","message":"Required field\.","context":\[\{"name":"provision\.device\.communicationModules\[\]\.identifier"}]},{"code":"0x010000","message":"Required field\.","context":\[\{"name":"provision\.device\.communicationModules\[\]\.subscriber\.identifier"}]},{"code":"0x010000","message":"Required field\.","context":\[\{"name":"provision\.device\.communicationModules\[\]\.identifier"}]},{"code":"0x010000","message":"Required field\.","context":\[\{"name":"provision\.device\.communicationModules\[\]\.subscriber\.identifier"}]}]\n
      """
  ...
  Scenario: I want to delete a device from json file
    ...
    Then the content of file "result.csv" must match:
      """
      statusCode;location;errors\n200;https:\/\/([\d\d\d]{1,3}\.){3}[\d\d\d]{1,3}:[\d\d\d]{1,4}\/v80\/provision\/organizations\/organization_bulk\/devices\/device_json_flattened;\n400;https:\/\/([\d\d\d]{1,3}\.){3}[\d\d\d]{1,3}:[\d\d\d]{1,4}\/v80\/provision\/organizations\/organization_bulk\/devices\/device_json_flattened_withCM;\[\{"code":"0x010006","message":"Entity not exists\.","context":\[\]}]\n
      """
