# features/provision/entities/create_delete_ticket_bulk_json_flattened_response_csv.feature
@provision
@create_provision
@tickets_provision
@bulk_ticket_json_flattened_response_csv
...
Feature: Delete and Create a ticket
  ...
  Scenario: I want to create a ticket from json file
    Given an ogapi "json flattened bulk builder" util with "organization_bulk" and "tickets"
    And I read the file from "/file_test/bulk_double_flattened_tickets.json" for provision tickets
    And I "create" it with bulk and response with format csv
    Then does not throws an error
    Then the content of file "result.csv" must match:
      """
      statusCode;location;errors\n201;https:\/\/([\d\d\d]{1,3}\.){3}[\d\d\d]{1,3}:[\d\d\d]{1,4}\/v80\/provision\/organizations\/organization_bulk\/tickets\/(\w|-)*;\n
      """
    Given an ogapi "json flattened bulk builder" util with "organization_bulk" and "tickets"
    And I read the file from "/file_test/bulk_double_flattened_tickets.json" for provision tickets
    ...
    Then the content of file "result.csv" must match:
      """
      statusCode;location;errors\n200;https:\/\/([\d\d\d]{1,3}\.){3}[\d\d\d]{1,3}:[\d\d\d]{1,4}\/v80\/provision\/organizations\/organization_bulk\/tickets\/(\w|-)*;\n
      """
    Given an ogapi "json flattened bulk builder" util with "organization_bulk" and "tickets"
    And I read the file from "/file_test/bulk_double_flattened_tickets.json" for provision tickets
