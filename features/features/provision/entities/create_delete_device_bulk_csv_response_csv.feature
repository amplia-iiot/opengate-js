...
Feature: Delete and Create a device
  ...
  Scenario: I want to create a device from csv file
    ...
    Then the content of file "result.csv" must match:
      """
      statusCode;location;errors\n201;https:\/\/([\d\d\d]{1,3}\.){3}[\d\d\d]{1,3}:[\d\d\d]{1,4}\/v80\/provision\/organizations\/organization_bulk\/devices\/device_bulk_c2;\n
      """
  ...
  Scenario: I want to create a device from csv file - duplicated error
    ...
    Then the content of file "result.csv" must match:
      """
      statusCode;location;errors\n400;https:\/\/([\d\d\d]{1,3}\.){3}[\d\d\d]{1,3}:[\d\d\d]{1,4}\/v80\/provision\/organizations\/organization_bulk\/devices\/device_bulk_c2;\[\{"code":"0x010114","message":"Entity duplicated\.","context":\[\]}]\n
      """
  ...
  Scenario: I want to update a device from csv file
    ...
    Then the content of file "result.csv" must match:
      """
      statusCode;location;errors\n200;https:\/\/([\d\d\d]{1,3}\.){3}[\d\d\d]{1,3}:[\d\d\d]{1,4}\/v80\/provision\/organizations\/organization_bulk\/devices\/device_bulk_c2;\n
      """
