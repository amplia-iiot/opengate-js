...
@urlParameters
#@fail_bulk
Feature: Delete and Create a device - bulk json and response csv
  ...
  Scenario: I want to create a device from json file
    ...
    Then the content of file "result.csv" must match:
      """
      statusCode;location;errors\n201;https:\/\/([\d\d\d]{1,3}\.){3}[\d\d\d]{1,3}:[\d\d\d]{1,4}\/v80\/provision\/organizations\/organization_bulk_10\/devices\/device_bulk_json_simple;\n
      """
  ...
  Scenario: I want to create a device from json file - duplicated error
    ...
    Then the content of file "result.csv" must match:
      """
      statusCode;location;errors\n400;https:\/\/([\d\d\d]{1,3}\.){3}[\d\d\d]{1,3}:[\d\d\d]{1,4}\/v80\/provision\/organizations\/organization_bulk_10\/devices\/device_bulk_json_simple;\[\{"code":"0x010114","message":"Entity duplicated\.","context":\[\]}]\n
      """
  ...
  Scenario: I want to update a device from json file
    ...
    Then the content of file "result.csv" must match:
      """
      statusCode;location;errors\n200;https:\/\/([\d\d\d]{1,3}\.){3}[\d\d\d]{1,3}:[\d\d\d]{1,4}\/v80\/provision\/organizations\/organization_bulk_10\/devices\/device_bulk_json_simple;\n
      """
